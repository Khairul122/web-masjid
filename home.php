...
<body>
				...
				<form action="tambah-transfer.php" method="POST" enctype="multipart/form-data">
					<table width="760" border="0" align="center" cellpadding="0" cellspacing="0">
						...
						<!-- <h1>
							...
							if (empty($_SESSION)) {
								echo "Tidak ada sesi yang tersedia.";
							} else {
								echo "<h3>Daftar Semua Sesi:</h3>";
								echo "<pre>";
								print_r($_SESSION);
								echo "</pre>";
							}
						...
						<tr height="46">
							...
							<td>
								<input
									...
									size="35"
									maxlength="6"
									value="<?php echo isset($_SESSION['nama_user']) ? htmlspecialchars($_SESSION['nama_user'], ENT_QUOTES, 'UTF-8') : ''; ?>"
						...
						<tr height="46">
							<td> </td>
							<td>Bank</td>
							<td><input name="bank" type="text" value="Bank BSI" class="form-control" readonly="readonly" /></td>
						</tr>
						<tr height="46">
							<td> </td>
							<td>Rekening</td>
							<td><input name="rekening" type="text" value="7181038777 a/n Mutiara Juwita" class="form-control" readonly="readonly" /></td>
						</tr>
						<tr height="46">
							<td> </td>
							<td>Tanggal</td>
    ...
    <tbody>
        <?php
        // Query untuk mendapatkan data kategori dan subtotal
        $query = "
            SELECT tbl_kategori.id_kategori, tbl_kategori.nama_kategori, tbl_sementaratrf.subtotal 
            FROM tbl_sementaratrf
        ...
        if ($tampil->num_rows > 0) {
            ...
            while ($row = $tampil->fetch_assoc()) {
                echo "
                <tr>
                    <td width='13%'>{$row['id_kategori']}</td>
                    <td width='13%'>{$row['nama_kategori']}</td>
                    <td width='17%'>Rp. " . str_replace(",", ".", number_format($row['subtotal'], 0)) . "</td>
                </tr>";
            }
        } else {
            // Jika tidak ada data
        ...
        // Query untuk mendapatkan total subtotal
        $totalQuery = "SELECT SUM(subtotal) as total FROM tbl_sementaratrf";
        $res = $koneksi->query($totalQuery);
            ...
            <tr class='info'>
                ...
                <td><b>
                    <form method='post' action='print.php'>
                        <input class='form-control' type='text' name='gtotal' readonly value='Rp. " . str_replace(",", ".", number_format($totalRow['total'], 0)) . "' />
                    </form>
                </b></td>
            </tr>";
        }
